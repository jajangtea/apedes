...
/**
 *
 * TCustomDatePicker class.
 *
 * TCustomDatePicker displays a text box for date input purpose.
 ...
 * </code>
 *
 * TCustomDatePicker has four <b>Mode</b> to show the date picker popup.
 *
...
class TCustomDatePicker extends TTextBox
{
	 ...
	 * @return integer the timestamp of the TCustomDatePicker control.
	 * @see getTimeStamp
	 * @since 3.1.2
	 */
	 ...
	 * @param integer the timestamp of the TCustomDatePicker control.
	 * @see setTimeStamp
	 * @since 3.1.2
	 */
	...
	/**
	 * Gets the TCustomDatePickerClientScript to set the TCustomDatePicker event handlers.
	 *
	...
	protected function renderCalendarDayOptions($writer, $selected=null)
	{
		...
		$writer->addAttribute('class', 'date-apedes date-apedes-day');
	...
	protected function renderCalendarMonthOptions($writer, $selected=null)
	{
		...
		$writer->addAttribute('class', 'date-apedes date-apedes-month');
		if($this->getReadOnly() || !$this->getEnabled(true))
			$writer->addAttribute('disabled', 'disabled');
		$writer->renderBeginTag('select');
	...
	protected function renderButtonDatePicker($writer)
	{
		...
		$writer->addAttribute('class', $this->getCssClass().' TCustomDatePickerButton');
	 ...
	 protected function renderImageButtonDatePicker($writer)
	{
		...
		$writer->addAttribute('class', $this->getCssClass().' TCustomDatePickerImageButton');
		if(!$this->getEnabled(true))
			$writer->addAttribute('disabled', 'disabled');
	...
	protected function renderClientControlScript($writer)
	{
		...
		{
			$cs = $this->getPage()->getClientScript();
			if(!$cs->isEndScriptRegistered('TCustomDatePicker.spacer'))
			{
				$spacer = $this->getAssetUrl('spacer.gif');
				$code = "Prado.WebUI.TDatePicker.spacer = '$spacer';";
				$cs->registerEndScript('TCustomDatePicker.spacer', $code);
			}
 ...
 * can be modified through the {@link TCustomDatePicker::getClientSide ClientSide}
 * property of a date picker.
 *
 * The <tt>OnDateChanged</tt> event is raise when the date picker's date
 * is changed.
 * The formatted date according to {@link TCustomDatePicker::getDateFormat DateFormat} is sent
 ...
 * that a {@link TCustomDatePicker} control can take.
 *
 * The following enumerable values are defined:
